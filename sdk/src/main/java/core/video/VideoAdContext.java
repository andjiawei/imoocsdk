...
import com.example.sdk.okhttp.HttpConstant;
import com.google.gson.Gson;

import adutils.Utils;
import core.AdContextInterface;
import module.AdValue;
...
public class VideoAdContext implements VideoAdSlot.AdSDKSlotListener {
    ...
    public VideoAdContext(ViewGroup parentView, String instance,
        ...
        this.mInstance=new Gson().fromJson(instance,AdValue.class);
    ...
    public void load() {
        if (mInstance != null && mInstance.resource != null) {
            ...
            //发送解析成功事件
            sendAnalizeReport(response.HttpConstant.Params.ad_analize, HttpConstant.AD_DATA_SUCCESS);
        } else {
            mAdSlot = new VideoAdSlot(null, this, mFrameLoadListener); //创建空的slot,不响应任何事件
            if (mListener != null) {
                mListener.onAdFailed();
            }
            sendAnalizeReport(response.HttpConstant.Params.ad_analize, HttpConstant.AD_DATA_FAILED);
        }
    }
    ...
    public void onAdVideoLoadSuccess() {
        ...
        sendAnalizeReport(response.HttpConstant.Params.ad_load, HttpConstant.AD_PLAY_SUCCESS);
    }
    ...
    public void onAdVideoLoadFailed() {
        ...
        sendAnalizeReport(response.HttpConstant.Params.ad_load, HttpConstant.AD_PLAY_FAILED);
    }
     ...
     * 发送广告数据解析成功监测
     */
    private void sendAnalizeReport(response.HttpConstant.Params step, String result) {
        try {
