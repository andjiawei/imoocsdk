...
import com.example.jiawei.imoocsdk.adapter.PhotoPagerAdapter;
import com.example.jiawei.imoocsdk.module.recommand.RecommandModel;
import com.example.jiawei.imoocsdk.view.viewpagerindictor.CirclePageIndicator;
...
public class HomeHeaderLayout extends RelativeLayout {
     ...
     * Data
     */
    private RecommandModel.DataBean.HeadBean mHeaderValue;
    private ImageLoaderUtil mImageLoader;

    public HomeHeaderLayout(Context context, RecommandModel.DataBean.HeadBean headerValue) {
        this(context, null, headerValue);
    }

    public HomeHeaderLayout(Context context, AttributeSet attrs, RecommandModel.DataBean.HeadBean headerValue) {
        super(context, attrs);
        mContext = context;
     ...
     private void initView() {
        ...
        mAdapter = new PhotoPagerAdapter(mContext, mHeaderValue.getAds(), true);
        mViewPager.setAdapter(mAdapter);
        mViewPager.startAutoScroll(3000);
        mPagerIndictor.setViewPager(mViewPager);

        for (int i = 0; i < mImageViews.length; i++) {
            mImageLoader.displayImage(mImageViews[i], mHeaderValue.getMiddle().get(i));
        }

        for (RecommandModel.DataBean.HeadBean.FooterBean value : mHeaderValue.getFooter()) {
            mFootLayout.addView(createItem(value));
        }
        mHotView.setText(mContext.getString(R.string.today_zuixing));
    }

    private HomeBottomItem createItem(RecommandModel.DataBean.HeadBean.FooterBean value) {
